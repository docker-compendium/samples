...
router.get("/openblogs", (req, res) => {
  req.app.get('db').collection('entries').find({ publicEntry: true })
    .project({ pic: 0 })
    .sort({ date: -1 })
...
router.get("/viewblog/:user", (req, res) => {
  ...
  req.app.get('db').collection('entries').find(query)
  ...
  Jimp.read(req.file.buffer).then(function (image) {
    image.cover(200, 200)
      .getBuffer(req.file.mimetype, function (err, buff) {
        ...
        req.app.get('db').collection('entries').insertOne(entry, { writeConcern: 1 }, (err, dbres) => {
...
router.get("/entry/:id", (req, res) => {
  ...
  req.app.get('db').collection('entries').findOne({ _id: new ObjectID(id) }, { projection: { pic: 0 } }, (err, entry) => {
    res.json(entry);
  });
});
router.patch("/entry/:id", [
  ...
  req.app.get('db').collection('entries').updateOne(
...
router.delete("/entry/:id", (req, res) => {
  ...
  req.app.get('db').collection('entries').deleteOne(entry, { w: 1 }, (err, dbres) => {
    res.json(dbres);
  });
});
router.get("/entrypic/:id", (req, res) => {
  const id = req.params.id;
  debug('trying to get entry ', id);
  req.app.get('db').collection('entries').findOne({ _id: new ObjectID(id) }, { projection: { pic: 1, title: 1, mime: 1 } }, (err, entry) => {
    res.json(entry);
  });
});
router.get("/viewimage/:id", (req, res) => {
  const id = req.params.id;
  req.app.get('db').collection('entries').findOne({ _id: new ObjectID(id) }, { projection: { pic: 1, mime: 1 } }, (err, entry) => {
    if (entry && entry.mime) {
